...
import { MediaType, SocialLinksType } from 'constants/types/settingsType.type'
import { useAtom } from 'jotai'
import { useEffect, useState } from 'react'
import { Cookies } from 'react-cookie'
import { axios } from 'utils/axios'
import { detectURLName, detectValidURL, getYoutubeThumbnail } from 'utils/utils'
...
export const Media = () => {
  ...
  const handleSubmit = async () => {
    const reformMediaForm = mediaForm.videoLinks?.map((link) => ({
      source: detectValidURL(link.url) ? detectURLName(link.url) : link.url,
      url: link.url,
